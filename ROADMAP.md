...
- [ ] **Roving tabindex för role=button-radlistor (samlat beslut, B7).** B2/B4 gav klickbara rader `role="button"` + `tabIndex={0}` (Preferences/ThemeEditor-nav, FileQueueItem). Fungerar, men ger ogiltig ARIA-nesting (interaktiva ättlingar i button-roll, WCAG 4.1.2) och många tab-stops i långa listor. Besluta ett gemensamt roving-tabindex-/listmönster och migrera alla förekomster i städ-PR B7 — inte per modul. Noterat i Nagelfar-granskningen av #174.
