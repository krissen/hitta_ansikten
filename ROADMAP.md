...
- [ ] **Startup-DB-preload kraschar mot borttagen `ManagementService.known_faces` (regression från #141).** `api/server.py`:s `_load_database_sync` avslutar med `return len(svc.known_faces)`, men D2-migreringen tog bort `ManagementService`:s egna kollektionsattribut. Vid boot fångas `AttributeError: 'ManagementService' object has no attribute 'known_faces'` av `preload_database`, som sätter `database`-komponenten till `error`/"Failed to load" i `/health` (statusen blir `degraded`). Appen fungerar ändå — store:t laddar DB:t lazy vid första riktiga anropet (verifierat: `/reload-database` returnerar 105 personer) — så det är enbart startup-statusräkningen som är trasig. Fix: hämta antalet via store:t, t.ex. `svc.store.read(lambda known, *_: len(known))`. Finns redan på `origin/dev`; upptäckt vid D3-verifiering. Hör till management/server-scope, inte D3 (detection).
