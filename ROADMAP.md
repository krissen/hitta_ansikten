...
- [ ] **B6+ — Större YOLO-variant + full pipeline på YOLO-detektioner. PÅBÖRJAD, AVBRUTEN 2026-07-09 — återupptagningsläge nedan.** `yolov8n-face` är nano-varianten; frågan som ska stängas är om en stor variant slår SCRFD där nano förlorade (Q1 +4,1 pp SCRFD). **Gjort:** `yolov8l-face` OCH `yolov12l-face` är engångs-ONNX-exporterade och ligger i arbetsträdet `.claude/worktrees/agent-a4c236f42efc4f898/backend/benchmarks/_data/models/` (flytta till huvudrepots `_data/models/` vid återupptagning); OCOMMITTADE kodändringar i samma worktree: `models/yoloface.py`, `models/download.py`, `models_manifest.json`, `detect_compare.py`, `run.py`, `test_benchmark_yoloface.py` (granska diffen innan användning — ingen commit hanns med, branchen `feature/benchmark-yolo-large` var ej skapad). **Kvar:** A/B-detektionsrecall stor-YOLO vs SCRFD på fulla datasetet (`detect_compare.py`; SCRFD-sidan cachad), end-to-end `benchmarks.run` med YOLO-detektor → buffalo_l (nano + stor), siffror till README/PR, bocka av denna post, **och stäng "PR 4b — Tiling"-posten ovan som avböjd** med hänvisning till auditrapportens dom (~99,8 % effektiv recall). Långkörningar nohup-frikopplade (harness-bundna bakgrundsjobb dödas). **Förutsättning:** `~/.local/share/faceid/benchmark_staging/` (~86 GB) behålls tills detta är klart — därefter väntar städningen på ägarens avrop. **Låg prioritet:** detektionen är inte flaskhalsen.
