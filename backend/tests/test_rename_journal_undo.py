...
def test_group_batches_orders_and_flags_undoable(journal, tmp_path):
    ...
    fs_ops.record(
        op="rename", tool="rename-nef", batch_id="b1", src=tmp_path / "a", dst=tmp_path / "b"
    )
    fs_ops.record(op="copy", tool="import", batch_id="b2", src=tmp_path / "c", dst=tmp_path / "d")
...
def test_api_batches_undoable_only_param(journal, tmp_path):
    ...
    fs_ops.record(
        op="rename", tool="rename-nef", batch_id="r1", src=tmp_path / "a", dst=tmp_path / "b"
    )
    fs_ops.record(op="copy", tool="import", batch_id="c1", src=tmp_path / "c", dst=tmp_path / "d")

    client = TestClient(app)
    default_ids = [
        b["batch_id"] for b in client.get("/api/v1/rename-journal/batches").json()["batches"]
    ]
    assert default_ids == ["r1"]  # copy filtered out by default
    all_ids = {
        b["batch_id"]
        for b in client.get("/api/v1/rename-journal/batches?undoable_only=false").json()["batches"]
    }
...
def test_partial_undo_skips_missing_and_reverts_rest(journal, tmp_path):
    ...
    fs_ops.record(
        op="rename", tool="rename", batch_id="p", src=tmp_path / "a1.NEF", dst=tmp_path / "b1.NEF"
    )
    fs_ops.record(
        op="rename", tool="rename", batch_id="p", src=tmp_path / "a2.NEF", dst=tmp_path / "b2.NEF"
    )
...
def test_undo_never_overwrites_occupied_original(journal, tmp_path):
    ...
    fs_ops.record(
        op="rename", tool="rename", batch_id="o", src=tmp_path / "a.NEF", dst=tmp_path / "b.NEF"
    )
...
def test_preview_matches_execute_chain_dest_freed_within_batch(journal, tmp_path):
    ...
    fs_ops.record(
        op="rename", tool="rename", batch_id="chain", src=tmp_path / "1.NEF", dst=tmp_path / "2.NEF"
    )
    fs_ops.record(
        op="rename", tool="rename", batch_id="chain", src=tmp_path / "2.NEF", dst=tmp_path / "3.NEF"
    )
...
def test_preview_matches_execute_chain_head_blocked_skips_cascade(journal, tmp_path):
    ...
    fs_ops.record(
        op="rename", tool="rename", batch_id="chain", src=tmp_path / "a.NEF", dst=tmp_path / "b.NEF"
    )
    fs_ops.record(
        op="rename", tool="rename", batch_id="chain", src=tmp_path / "b.NEF", dst=tmp_path / "c.NEF"
    )
...
async def test_service_refuses_non_undoable_batch(journal, tmp_path):
    ...
    fs_ops.record(op="copy", tool="import", batch_id="cp", src=tmp_path / "s", dst=tmp_path / "d")
    svc = UndoService()
    with pytest.raises(ValueError, match="kan inte ångras"):
...
async def test_service_preview_then_execute(journal, tmp_path, monkeypatch):
    ...
    (tmp_path / "b.NEF").write_bytes(b"data")
    fs_ops.record(
        op="rename", tool="rename-nef", batch_id="x", src=tmp_path / "a.NEF", dst=tmp_path / "b.NEF"
    )
...
async def test_undo_repairs_known_faces_and_processed_paths(journal, tmp_path, monkeypatch):
    ...
    faceid_db.save_database(
        {"Anna": [_face_entry(img)]}, [], {}, [{"name": str(img), "hash": "h1"}]
    )
...
async def test_undo_db_repair_matches_full_path_not_basename(journal, tmp_path, monkeypatch):
    ...
    fs_ops.record(op="rename", tool="rename", batch_id="b", src=d1 / "IMG_0001.NEF", dst=renamed1)
...
async def test_undo_db_repair_handles_basename_processed_entries(journal, tmp_path, monkeypatch):
    ...
    faceid_db.save_database(
        {"Anna": [_face_entry(renamed)]}, [], {}, [{"name": "250101_120000_Anna.NEF", "hash": "h1"}]
    )  # BARE basename

    fs_ops.record(
        op="rename", tool="rename", batch_id="b", src=tmp_path / "IMG_0001.NEF", dst=renamed
    )
    ...
    known_file, proc_name = db_store_mod.get_db_store().read(
        lambda known, ignored, hardneg, processed: (known["Anna"][0]["file"], processed[0]["name"])
    )
...
def test_api_batches_and_undo(journal, tmp_path, monkeypatch):
    ...
    resp = client.post("/api/v1/rename-journal/undo", json={"batch_id": "api1", "execute": True})
...
def test_api_undo_unknown_batch_404(journal, tmp_path):
    ...
    client = TestClient(app)
    resp = client.post("/api/v1/rename-journal/undo", json={"batch_id": "missing", "execute": True})
