        ...
        if not should_process:
            ...
            if cached:
                try:
                    ...
                    pass  # evicted between lookup and read — fall through to error
            return {'status': 'error', 'error': 'Thumbnail generation failed in another thread'}

        try:
            jpg_data = _make_preview_thumb_sync(file_path, size)
            # Return the freshly generated bytes directly. store_grid_thumb runs
            # LRU eviction, which on a tiny cache could remove the file we just
            # wrote — so we never rely on re-reading it from disk.
