...
async def lifespan(app: FastAPI):
    ...
    async def preload_database():
        ...
        try:
            ...
            startup_state.set_state("database", LoadingState.READY, f"{people_count} persons")
            logger.info(f"[Startup Profile] Database loaded in {elapsed:.2f}s")
        except Exception as e:
            logger.exception(f"Failed to pre-load database: {e}")
            startup_state.set_state("database", LoadingState.ERROR, "Failed to load", error=str(e))
    ...
    async def eager_load_ml():
        ...
        try:
            ...
            startup_state.set_state("mlModels", LoadingState.READY, f"Ready ({elapsed:.1f}s)")
            ...
            startup_state.set_state("mlModels", LoadingState.ERROR, "Failed to load", error=str(e))
...
async def health_check():
    ...
    return {
        ...
        "version": app.version,
        "components": {
            name: {"state": info["state"], "message": info["message"]}
...
app.include_router(
    preprocessing.router, prefix=f"{API_V1_PREFIX}/preprocessing", tags=["preprocessing"]
)
