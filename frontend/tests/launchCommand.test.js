...
describe('resolveLaunchCommand — culling', () => {
  it('open-culling with expanded roots', async () => {
    ...
    const { command } = await resolveLaunchCommand(
      args({ verb: 'culling', files: ['/events/cupen'], recursive: true }), d);
  ...
  it('paths that expand to nothing → enter-step culling EMPTY (the bug fix, not a strand)', async () => {
    ...
    const { command } = await resolveLaunchCommand(
      args({ verb: 'culling', files: ['/typo'] }), d);
  ...
  it('bare --clear with no roots still opens culling (empties it)', async () => {
    const d = deps({ folders: [] });
    const { command } = await resolveLaunchCommand(
      args({ verb: 'culling', clear: true }), d);
    expect(command).toEqual({ type: 'open-culling', payload: { roots: [], clear: true, recursive: false } });
  });
});
...
describe('resolveLaunchCommand — import', () => {
  it('always opens import, destination optional', async () => {
    ...
    const { command } = await resolveLaunchCommand(args({ verb: 'import', files: ['/dest'] }), d);
    expect(command).toEqual({ type: 'open-import', payload: { destination: '/dest' } });
  });
});
...
describe('resolveLaunchCommand — faces / legacy', () => {
  it('queue flag → queue-files with expanded files', async () => {
    ...
    const { command } = await resolveLaunchCommand(
      args({ verb: 'faces', files: ['*.nef'], queuePosition: 'end', startQueue: true }), d);
    expect(command).toEqual({
      type: 'queue-files',
      payload: { files: ['/a.nef', '/b.nef'], position: 'end', startQueue: true, clear: false },
    });
  });
  ...
  it('paths that expand to nothing → enter-step review EMPTY (not a strand)', async () => {
    const d = deps({ files: [] });
    const { command } = await resolveLaunchCommand(
      args({ verb: 'faces', files: ['/typo.nef'], queuePosition: 'end', startQueue: true }), d);
  ...
  it('multiple files, no queue flag → queue-files without auto-start', async () => {
    const d = deps({ files: ['/a.nef', '/b.nef'] });
    const { command } = await resolveLaunchCommand(
      args({ files: ['/a.nef', '/b.nef'] }), d);
    expect(command).toEqual({
      type: 'queue-files',
      payload: { files: ['/a.nef', '/b.nef'], position: 'end', startQueue: false, clear: false },
    });
  });
