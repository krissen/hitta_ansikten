...
const CASES = [
  ...
  [
    'side / measured side column present',
    MEASURED_TWO,
    'side',
    { tabsetId: 'left' },
  ],
  [
    'side / unmeasured side column present',
    UNMEASURED_TWO,
    'side',
    { tabsetId: 'left' },
  ],
  [
    'side / solo (no side column)',
    SOLO,
    'side',
    { split: 'left', refTabsetId: 'only' },
  ],
  [
    'side / Database narrow non-side column → split, not hijack',
    DATABASE,
    'side',
    { split: 'left', refTabsetId: 'stats' },
  ],

  // bottom → reuse a measured bottom bar hosting a bottom module, else split.
  [
    'bottom / measured bar present',
    MEASURED_BOTTOM,
    'bottom',
    { tabsetId: 'bar' },
  ],
  [
    'bottom / measured no bar',
    MEASURED_TWO,
    'bottom',
    { split: 'bottom', refTabsetId: 'main' },
  ],
  [
    'bottom / measured panel below hosts non-bottom → split',
    BOTTOM_NONBOTTOM,
    'bottom',
    { split: 'bottom', refTabsetId: 'main' },
  ],
  [
    'bottom / unmeasured (no measured bar)',
    UNMEASURED_TWO,
    'bottom',
    { split: 'bottom', refTabsetId: 'main' },
  ],
...
describe('split weight invariant (new side/bottom pane < main)', () => {
  it('side split from a solo workspace: pane 15, main 85', () => {
    ...
    applyPlacement(
      model,
      { type: 'tab', name: 'Queue', component: 'file-queue' },
      placement,
      15,
    );
    ...
    expect(weightOfTabset(model, paneId)).toBeLessThan(
      weightOfTabset(model, mainId),
    );
  });
  ...
  it('bottom split from a solo workspace: pane 20, main 80', () => {
    ...
    applyPlacement(
      model,
      { type: 'tab', name: 'Logs', component: 'log-viewer' },
      placement,
      20,
    );
  ...
  it('does not resize when docking into an existing tabset (no split)', () => {
    ...
    applyPlacement(
      model,
      { type: 'tab', name: 'Stats', component: 'statistics-dashboard' },
      placement,
      getModuleWeight('statistics-dashboard'),
    );
