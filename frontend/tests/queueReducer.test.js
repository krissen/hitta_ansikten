...
import {
  ...
  insertItems,
  initialQueueState,
} from '../src/renderer/components/fileQueue/queueReducer.js';
...
describe('queueReducer', () => {
  it('restore replaces the queue and resets currentIndex', () => {
    const next = queueReducer(state([item('x')], 3), {
      type: 'restore',
      queue: [item('a')],
    });
  ...
  it('add inserts deduped items', () => {
    const next = queueReducer(state([item('a')]), {
      type: 'add',
      items: [item('b')],
      position: 'end',
    });
  ...
  it('sort natural-sorts', () => {
    const next = queueReducer(
      state([
        item('b', { fileName: 'b.NEF' }),
        item('a', { fileName: 'a.NEF' }),
      ]),
      { type: 'sort' },
    );
  ...
  it('removePaths drops matching paths, leaves index', () => {
    const next = queueReducer(state([item('a'), item('b')], 1), {
      type: 'removePaths',
      paths: new Set(['/p/a.NEF']),
    });
  ...
  it('clear empties and resets index', () => {
    expect(queueReducer(state([item('a')], 0), { type: 'clear' })).toEqual({
      queue: [],
      currentIndex: -1,
    });
  });
  ...
  it('clearDone drops completed and already-processed (fix-mode off)', () => {
    ...
    const next = queueReducer(state(q, 2), {
      type: 'clearDone',
      fixMode: false,
      visibleIds: null,
    });
  ...
  it('markProcessedByNames marks by fileName and no-ops when nothing changes', () => {
    ...
    const next = queueReducer(s, {
      type: 'markProcessedByNames',
      names: new Set(['a.NEF']),
    });
    expect(next.queue[0].isAlreadyProcessed).toBe(true);
    const same = queueReducer(s, {
      type: 'markProcessedByNames',
      names: new Set(['b.NEF']),
    });
  ...
  it('applyRename updates filePath and fileName', () => {
    const next = queueReducer(state([item('a')]), {
      type: 'applyRename',
      renamedMap: { '/p/a.NEF': '/p/a_Alice.NEF' },
    });
    expect(next.queue[0]).toMatchObject({
  ...
  it('insertAt re-inserts at an index, and is a no-op if already present', () => {
    const next = queueReducer(state([item('a'), item('c')]), {
      type: 'insertAt',
      item: item('b'),
      index: 1,
    });
    expect(next.queue.map((i) => i.id)).toEqual(['a', 'b', 'c']);
    const same = queueReducer(next, {
      type: 'insertAt',
      item: item('b'),
      index: 0,
    });
  ...
  it('setIndex sets currentIndex and no-ops when unchanged', () => {
    ...
    expect(queueReducer(s, { type: 'setIndex', index: 2 }).currentIndex).toBe(
      2,
    );
