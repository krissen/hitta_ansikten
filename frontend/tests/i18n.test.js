...
describe('i18n t()', () => {
  ...
  it('has the Swedish application-menu labels', () => {
    ...
    expect(t('menu.file.openInLightroom')).toBe(
      'Öppna original i extern editor',
    );
    expect(t('menu.app.aboutDetail', { version: 'dev' })).toContain(
      'Version: dev',
    );
  });
  ...
  it('resolves the external-editor strings (a typo would render the raw key)', () => {
    ...
    expect(
      t('culling.errors.editorFailed', { editor: 'Capture One', detail: '' }),
