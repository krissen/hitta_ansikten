...
import { ProgressBar, LoadingOverlay } from '../../src/renderer/components/shared/ProgressBar.jsx';
...
describe('ProgressBar', () => {
  ...
  it('sets the fill width from the value', () => {
    ...
    expect(container.querySelector('.progress-bar__fill').style.width).toBe('75%');
  });

  it('omits aria-valuenow in indeterminate mode', () => {
    const { container } = render(<ProgressBar />);
    const bar = container.querySelector('[role="progressbar"]');
    expect(container.querySelector('.progress-bar').classList.contains('progress-bar--indeterminate')).toBe(true);
  ...
  it('uses an explicit ariaLabel when provided', () => {
    const { container } = render(<ProgressBar value={10} ariaLabel="Importerar" />);
    expect(container.querySelector('[role="progressbar"]').getAttribute('aria-label')).toBe('Importerar');
  });

  it('falls back to a string label for the aria-label', () => {
    const { container } = render(<ProgressBar value={10} label="Bearbetar" />);
    expect(container.querySelector('[role="progressbar"]').getAttribute('aria-label')).toBe('Bearbetar');
  });

  it('shows a rounded percent when requested', () => {
    const { container } = render(<ProgressBar value={33.6} showPercent />);
    expect(container.querySelector('.progress-bar__percent').textContent).toBe('34%');
  });
});
...
describe('LoadingOverlay', () => {
  ...
  it('renders nothing when not visible', () => {
    const { container } = render(<LoadingOverlay visible={false} message="x" />);
