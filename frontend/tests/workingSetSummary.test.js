...
const build = (sets) => buildWorkingSetSummary(t, { steps: WORKFLOW_STEPS, ...sets });
...
describe('buildWorkingSetSummary', () => {
  ...
  it('renders empty states when nothing is set', () => {
    const [queue, scan, anchor] = build({ queue: null, scan: null, anchor: null });
  ...
  it('queue: "N filer (M klara) — <mapp>" from the basename of the folder', () => {
    const [queue] = build({ queue: { folder: '/events/cupen', count: 12, done: 5 } });
  ...
  it('scan: folder basename plus recursive/glob/date flags', () => {
    const [, scan] = build({
      scan: { roots: ['/a/cup'], globs: ['*x*'], recursive: true, date_from: '2025-01-01' },
    });
  ...
  it('scan: a glob-only scope shows the glob as the "where"', () => {
    const [, scan] = build({ scan: { roots: [], globs: ['/p/*.jpg'], recursive: false } });
  ...
  it('anchor: shows "satt av <steg>" for a known step, plain folder otherwise', () => {
    const [, , withStep] = build({ anchor: { roots: ['/e/cupen'], step: 'rename' } });
