...
describe('resolveSuggestion', () => {
  it('returns null when there is no top alternative', () => {
    ...
    expect(
      resolveSuggestion(face({ match_alternatives: undefined })),
    ).toBeNull();
  ...
  it('resolves the normal (name) case from the first alternative', () => {
    ...
    expect(resolveSuggestion(f)).toEqual({
      name: 'Anna',
      confidence: 82,
      isIgnore: false,
    });
  ...
  it('flags isIgnore when the top alternative is is_ignored', () => {
    const f = face({
      match_alternatives: [
        { name: 'Skomakare', confidence: 60, is_ignored: true },
      ],
    });
    expect(resolveSuggestion(f)).toEqual({
      name: 'Skomakare',
...
describe('getTopMatch', () => {
  it('returns the top alternative at or above the 75% threshold', () => {
    const f = face({
      match_alternatives: [{ name: 'Anna', confidence: 75, is_ignored: false }],
    });
  ...
  it('returns null below the threshold, for ignore-suggestions, and without alternatives', () => {
    expect(
      getTopMatch(
        face({ match_alternatives: [{ name: 'Anna', confidence: 74 }] }),
      ),
    ).toBeNull();
    expect(
      getTopMatch(
        face({
          match_alternatives: [
            { name: 'ign', confidence: 90, is_ignored: true },
          ],
        }),
      ),
    ).toBeNull();
...
describe('confirmFaceState — batch-confirm confirmation entry (per-field)', () => {
  it('builds the exact confirmation payload fields', () => {
    ...
    const { confirmation } = confirmFaceState(
      faces,
      0,
      '  Chosen Name  ',
      '/photos/a.jpg',
  ...
  it('marks the face confirmed with the trimmed name and snapshots an undo entry', () => {
    ...
    expect(result.undoEntry).toEqual({
      type: 'confirm',
      index: 0,
      face: faces[0],
    });
  ...
  it('is a no-op (null) for missing or already-confirmed faces', () => {
    ...
    expect(
      confirmFaceState([face({ is_confirmed: true })], 0, 'Anna', '/p.jpg'),
    ).toBeNull();
  });
});
...
describe('ignoreFaceState — batch-confirm ignore entry (per-field)', () => {
  ...
  it('is a no-op (null) for already-confirmed faces', () => {
    expect(
      ignoreFaceState([face({ is_confirmed: true })], 0, '/p.jpg'),
    ).toBeNull();
  });
});
...
describe('unconfirmFaceState / undoFaceState', () => {
  ...
  it('undo restores the snapshotted face at its index', () => {
    ...
    const faces = [
      face({ face_id: 'x', is_confirmed: true, person_name: 'Anna' }),
    ];
    const restored = undoFaceState(faces, {
      type: 'confirm',
      index: 0,
      face: original,
    });
...
describe('acceptAllState', () => {
  it('confirms named suggestions, ignores ign-suggestions, skips faces without alternatives', () => {
    const faces = [
      face({
        face_id: 'a',
        match_alternatives: [{ name: ' Anna ', confidence: 80 }],
      }),
      face({
        face_id: 'b',
        match_alternatives: [{ name: 'ign', confidence: 70, is_ignored: true }],
      }),
...
describe('pending-list helpers', () => {
  it('upsertConfirmation updates person_name in place, keyed by face_id', () => {
    ...
    const next = upsertConfirmation(prev, {
      face_id: 'a',
      person_name: 'New',
      suggested_name: null,
    });
    expect(next).toEqual([
      { face_id: 'a', person_name: 'New', suggested_name: 'S' },
    ]);
    expect(
      upsertConfirmation([], { face_id: 'b', person_name: 'X' }),
    ).toHaveLength(1);
  });

  it('mergeConfirmations also replaces suggested_name (accept-all semantics)', () => {
    const prev = [{ face_id: 'a', person_name: 'Old', suggested_name: 'S' }];
    const next = mergeConfirmations(prev, [
      { face_id: 'a', person_name: 'New', suggested_name: null },
    ]);
    expect(next).toEqual([
      { face_id: 'a', person_name: 'New', suggested_name: null },
    ]);
  ...
  it('appendIgnore and mergeIgnores skip duplicates by face_id', () => {
    ...
    expect(appendIgnore(prev, { face_id: 'a', image_path: '/p.jpg' })).toBe(
      prev,
    );
    expect(
      appendIgnore(prev, { face_id: 'b', image_path: '/p.jpg' }),
    ).toHaveLength(2);
    expect(
      mergeIgnores(prev, [{ face_id: 'a' }, { face_id: 'b' }]),
    ).toHaveLength(2);
  });
});
