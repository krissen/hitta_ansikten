...
const VARIABLE_GROUPS = {
  ...
  'Tab Appearance': {
    ...
    variables: [
      {
        name: 'tabsHeight',
        label: 'themeEditor.vars.tabs.height',
        min: 20,
        max: 40,
        unit: 'px',
        pref: 'appearance.tabsHeight',
      },
      {
        name: 'tabsFontSize',
        label: 'themeEditor.vars.tabs.fontSize',
        min: 10,
        max: 18,
        unit: 'px',
        pref: 'appearance.tabsFontSize',
      },
      {
        name: 'tabPaddingLeft',
        label: 'themeEditor.vars.tabs.paddingLeft',
        min: 0,
        max: 20,
        unit: 'px',
        pref: 'appearance.tabPaddingLeft',
      },
      {
        name: 'tabPaddingRight',
        label: 'themeEditor.vars.tabs.paddingRight',
        min: 0,
        max: 20,
        unit: 'px',
        pref: 'appearance.tabPaddingRight',
      },
      {
        name: 'tabMinGap',
        label: 'themeEditor.vars.tabs.minGap',
        min: 0,
        max: 30,
        unit: 'px',
        pref: 'appearance.tabMinGap',
      },
      {
        name: 'tabMinWidth',
        label: 'themeEditor.vars.tabs.minWidth',
        min: 0,
        max: 200,
        unit: 'px',
        pref: 'appearance.tabMinWidth',
      },
    ],
  },
  Backgrounds: {
  ...
  'Accent Colors': {
    ...
    variables: [
      { name: '--accent-primary', label: 'themeEditor.vars.accent.primary' },
      {
        ...
        label: 'themeEditor.vars.accent.primaryHover',
      },
      {
        name: '--accent-secondary',
        label: 'themeEditor.vars.accent.secondary',
      },
      {
        name: '--accent-secondary-hover',
        label: 'themeEditor.vars.accent.secondaryHover',
      },
    ],
  },
  'Semantic Colors': {
    ...
    variables: [
      { name: '--color-success', label: 'themeEditor.vars.semantic.success' },
      {
        ...
        label: 'themeEditor.vars.semantic.successBg',
      },
      { name: '--color-warning', label: 'themeEditor.vars.semantic.warning' },
      {
        name: '--color-warning-bg',
        label: 'themeEditor.vars.semantic.warningBg',
      },
      ...
      { name: '--color-info-bg', label: 'themeEditor.vars.semantic.infoBg' },
      {
        ...
        label: 'themeEditor.vars.semantic.successText',
      },
      {
        name: '--color-warning-text',
        label: 'themeEditor.vars.semantic.warningText',
      },
      {
        name: '--color-error-text',
        label: 'themeEditor.vars.semantic.errorText',
      },
      {
        name: '--color-info-text',
        label: 'themeEditor.vars.semantic.infoText',
      },
    ],
  },
  'Overlay Colors': {
  ...
  Spacing: {
    ...
    variables: [
      {
        name: '--space-xs',
        label: 'themeEditor.sizes.extraSmall',
        min: 0,
        max: 16,
      },
      ...
      {
        name: '--space-md',
        label: 'themeEditor.sizes.medium',
        min: 0,
        max: 32,
      },
      ...
      {
        name: '--space-xl',
        label: 'themeEditor.sizes.extraLarge',
        min: 0,
        max: 64,
      },
      {
        name: '--space-2xl',
        label: 'themeEditor.sizes.xxLarge',
        min: 0,
        max: 96,
      },
    ],
  ...
  Opacity: {
    ...
    variables: [
      {
        name: '--toast-opacity',
        label: 'themeEditor.vars.opacity.toast',
        min: 0.5,
        max: 1.0,
        step: 0.01,
      },
      {
        ...
        label: 'themeEditor.vars.opacity.overlay',
        min: 0.5,
        max: 1.0,
        step: 0.01,
      },
...
export function ThemeEditor({ api }) {
  ...
  const handlePresetBindingChange = useCallback(
    (themeType, presetName) => {
      ...
      if (BUILTIN_PRESETS[name]) {
        ...
        Object.entries(preset).forEach(([varName, value]) => {
          ...
          if (group) {
            ...
            setCSSVariable(
              varName,
              group.type === 'number' ? `${value}${unit}` : value,
            );
          }
        });
        setValues(preset);
        ...
        <div className="theme-mode-section">
          <div className="form-field theme-mode-selector">
            <label className="form-label">
              {t('themeEditor.labels.currentTheme')}
            </label>
            <select
            ...
            >
              <option value="light">
                {t('themeEditor.themeModeOptions.light')}
              </option>
              <option value="dark">
                {t('themeEditor.themeModeOptions.dark')}
              </option>
              <option value="system">
                {t('themeEditor.themeModeOptions.system')}
              </option>
            </select>
            <p className="hint-text">
              {themeMode === 'system'
                ? t('themeEditor.hints.followingSystem', {
                    theme: t(
                      `themeEditor.themeModeOptions.${themeManager.getCurrentTheme()}`,
                    ),
                  })
                : t('themeEditor.hints.usingTheme', {
                    theme: t(`themeEditor.themeModeOptions.${themeMode}`),
                  })}
            </p>
          </div>
          ...
          <div className="section-card preset-bindings">
            <h4 className="section-title">
              {t('themeEditor.labels.presetBindings')}
            </h4>
            <p className="hint-text">{t('themeEditor.hints.presetBindings')}</p>
            <div className="binding-row">
              <label className="form-label">
                {t('themeEditor.labels.lightModePreset')}
              </label>
              <select
                className="form-select"
                value={presetBindings.light}
                onChange={(e) =>
                  handlePresetBindingChange('light', e.target.value)
                }
              >
            ...
            <div className="binding-row">
              <label className="form-label">
                {t('themeEditor.labels.darkModePreset')}
              </label>
              <select
                className="form-select"
                value={presetBindings.dark}
                onChange={(e) =>
                  handlePresetBindingChange('dark', e.target.value)
                }
              >
                {allPresetNames.map((name) => (
                  <option key={name} value={name}>
              ...
              <div key={v.name} className="variable-item number-item">
                ...
                <div className="number-input-wrapper">
                  <input
                    ...
                    value={value}
                    onChange={(e) =>
                      handleTabValueChange(v.name, e.target.value, v.pref)
                    }
                  />
                  <input
                    type="number"
              ...
              <div key={v.name} className="variable-item color-item">
                ...
                <div className="color-input-wrapper">
                  <input
                    ...
                    onChange={(e) =>
                      handleValueChange(v.name, e.target.value, 'color')
                    }
                  />
                  <input
                    type="text"
  ...
  return (
    <div
      className="module-container has-sidebar theme-editor"
    ...
    >
        ...
        <div className="sidebar-actions">
          ...
          <div className="preset-buttons">
            <Button variant="secondary" onClick={exportPresets}>
              {t('themeEditor.buttons.export')}
            </Button>
            <Button variant="secondary" onClick={importPresets}>
              {t('themeEditor.buttons.import')}
            </Button>
      ...
      <div className="module-content">
        <h2 className="content-title">
          {t(VARIABLE_GROUPS[activeGroup].titleKey)}
        </h2>
