...
export function useFileQueue() {
  const [{ queue, currentIndex }, dispatch] = useReducer(
    queueReducer,
    initialQueueState,
  );
  ...
  const actions = useMemo(
    () => ({
      ...
      clearDone: (fixMode, visibleIds) =>
        dispatch({ type: 'clearDone', fixMode, visibleIds }),
      clearSelected: (selectedIds) =>
        dispatch({ type: 'clearSelected', selectedIds }),
      setActive: (index) => dispatch({ type: 'setActive', index }),
      clearProcessedFlag: (index) =>
        dispatch({ type: 'clearProcessedFlag', index }),
      ...
      markProcessedByNames: (names) =>
        dispatch({ type: 'markProcessedByNames', names }),
      applyRename: (renamedMap) =>
        dispatch({ type: 'applyRename', renamedMap }),
