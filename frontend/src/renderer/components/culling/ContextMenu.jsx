  ...
  return (
    ...
    >
      <li
        onClick={() => {
          setMenu(null);
          guardedNavigate(() => setCurrentIndex((i) => Math.max(i - 1, 0)));
        }}
      >
        {/* In the grid ↑/↓ move by rows, so the prev/next hint is just ←/→ there. */}
        <span>{t('culling.contextMenu.previous')}</span>
        <span className="culling-menu-keys">
          <kbd>←</kbd>
          {viewMode !== 'grid' && <kbd>↑</kbd>}
        </span>
      </li>
      <li
        onClick={() => {
      ...
      >
        <span>{t('culling.contextMenu.next')}</span>
        <span className="culling-menu-keys">
          <kbd>→</kbd>
          {viewMode !== 'grid' && <kbd>↓</kbd>}
        </span>
      ...
      <li
        onClick={() => {
          setMenu(null);
          guardedNavigate(() =>
            setCurrentIndex((i) => Math.max(i - pageStep, 0)),
          );
        }}
      >
        <span>{t('culling.contextMenu.jumpBack')}</span>
        <span className="culling-menu-keys">
          <kbd>⌥</kbd>
          <kbd>←</kbd>
        </span>
      ...
      <li
        onClick={() => {
          setMenu(null);
          guardedNavigate(() =>
            setCurrentIndex((i) => Math.min(i + pageStep, files.length - 1)),
          );
        }}
      >
        <span>{t('culling.contextMenu.jumpForward')}</span>
        <span className="culling-menu-keys">
          <kbd>⌥</kbd>
          <kbd>→</kbd>
        </span>
      ...
      >
        <span>{t('culling.contextMenu.rename')}</span>
        <span className="culling-menu-keys">
          <kbd>Enter</kbd>
        </span>
      </li>
      ...
      >
        <span>{t('culling.contextMenu.cull')}</span>
        <span className="culling-menu-keys">
          <kbd>X</kbd>
          <kbd>⌘</kbd>
          <kbd>⌫</kbd>
      ...
      >
        <span>{t('culling.contextMenu.undoLast')}</span>
        <span className="culling-menu-keys">
          <kbd>⌘</kbd>
          <kbd>Z</kbd>
        </span>
      ...
      <li
        onClick={() => {
          const p = menu.path;
          setMenu(null);
          openRawInLightroom(p);
        }}
      >
        <span>{t('culling.contextMenu.openInLightroom')}</span>
        <span className="culling-menu-keys">
          <kbd>L</kbd>
        </span>
      </li>
